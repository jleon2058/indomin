...
class AccountMove(models.Model):
    ...
    def _inverse_amount_total(self):
        for move in self:
            ...
            to_force_exchange_rate = move.to_force_exchange_rate
            force_flag = to_force_exchange_rate != 0.0 and move.company_id.currency_id != move.currency_id
            if force_flag:
                balance = move.currency_id._force_convert(amount_currency, move.company_currency_id, move.company_id, to_force_exchange_rate)
            else:
...
#                 .filtered(lambda line: line.account_id.account_type in ('receivable', 'payable'))
...
#                     force_flag = to_force_exchange_rate != 0.0 and move.company_id.currency_id != move.currency_id
#                     if force_flag:
#                         amount = move.company_currency_id._force_convert(
...
#                             move.currency_id,
#                             move.company_id,
#                             move.to_force_exchange_rate if move.to_force_exchange_rate else line.move_id.invoice_date or line.date,
#                         )
...
#                 force_flag = to_force_exchange_rate != 0.0 and self.company_id.currency_id != self.currency_id
#                 if force_flag:
#                     diff_balance = self.currency_id._force_convert(diff_amount_currency, self.company_currency_id, self.company_id, to_force_exchange_rate)
#                 else:
...
#         others_lines = self.line_ids.filtered(lambda line: line.account_id.account_type not in ('receivable', 'payable'))
...
#     # TODO migrar codigo a la v15, solo se ha colocado el encabezado actualmente
...
#             force_flag = to_force_exchange_rate != 0.0 and self.company_id.currency_id != self.currency_id
#             if force_flag:
#                 tax_base_amount = currency._force_convert(taxes_map_entry['tax_base_amount'], self.company_currency_id, self.company_id, to_force_exchange_rate)
#             else:
...
#             force_flag = to_force_exchange_rate != 0.0 and self.company_id.currency_id != self.currency_id
#             if force_flag:
#                 balance = currency._force_convert(
...
#         """
#             @api.onchange('currency_id', 'to_force_exchange_rate','invoice_line_ids','invoice_line_ids.price_subtotal','invoice_line_ids.tax_ids')
